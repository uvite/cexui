  ...
  useEffect(
    (priceScaleId: string) => {
      ...
      if (!hasSymbol && markets.length > 0) {
        ...
        setSymbol((markets.find((m) => m.symbol.includes('BTC')) || markets[0]).symbol);
      }
        ...
        connector
          ...
          .then((candles) => {
            if (symbol === symbolRef.current && interval === intervalRef.current) {
              ...
              volumeSeries?.setData(candles.map(({ time, volume: value }) => ({ time, value })));

              if (candles.length) {
                const fromCandlesIdx = candles.length - 150 > 0 ? candles.length - 150 : 0;

                chart?.priceScale(priceScaleId).applyOptions({ autoScale: true });
